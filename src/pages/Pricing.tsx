        ...
        <div className="min-h-screen bg-cream p-6 md:p-12 text-brandBlack">
            <Section className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {plans.map((plan, i) => (
                        <div key={i} className={`relative p-8 rounded-[40px] border-2 border-brandBlack shadow-xl flex flex-col ${plan.color}`}>
                            {plan.popular && (
                            ...
                            <ul className="space-y-4 mb-12 flex-1">
                                {plan.features.map((feature, fi) => (
                                    <li key={fi} className="flex items-center gap-3 text-sm font-bold">
                                        <Check className="w-5 h-5 text-brandGreen flex-shrink-0" />
                                        {feature}
                                    </li>
                                ))}
                            </ul>
